...
**1a-0 은 이제 Q42 가 막는다** (004 §8-4 — `integration/pleiades` 가 `dev` 로 가지 않으므로 003 의 실서비스 배포·검증 전제가 무너졌다. 1a 변경을 어디서 띄워 Discord 알림을 검증할지 먼저 정한다). **1a-1 부터는 추가로 하네스 통합(#1)이 선결**이고 #1 은 방향만 확정됐다 — 1a-1 이 이미 두 저장소 코드를 흡수한다.
...
1a 착수 전 남은 것 — **단계별로 무엇이 막는지는 `003` §10-1 의존 지도가 정본이다** (1a-0 은 **Q42**(004 §8-4) · 1a-2 는 **#1 + Q16·Q17·Q18** · 1a-1·1a-3·1a-4 는 Q10 외 다수):

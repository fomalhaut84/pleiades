...
**현재 상태: 방향 확정 · 배치·룰 완료 · 하네스 통합(#1) 집행 — **pleiades PR 5개(#19·#20·#22·#24·#26) · myFinance#492 · myFitness#369 전부 머지됨**(2026-09-07). 남은 것 H-1(Q41) · H-3(fit) · 단계 0 미착수 · 단계 1(1a) 착수 대기.**
...
**1a-0 은 착수 가능하다 — Q42 확정(#29):** 검증은 로컬 `repos/*` + **서버 병행 인스턴스(읽기 전용 DB 롤 · 별도 포트·pm2 · Nginx 미연결 · 봇 미기동 · cron off — 6조건)**. 착수 시 1a 발송 경로의 DB 쓰기 지점을 먼저 잰다(003 §10-1). 서버 작업은 별도 승인. **1a-1 부터는 추가로 하네스 통합(#1)이 선결**이고 #1 은 방향만 확정됐다 — 1a-1 이 이미 두 저장소 코드를 흡수한다.
...
1a 착수 전 남은 것 — **단계별로 무엇이 막는지는 `003` §10-1 의존 지도가 정본이다** (1a-0 은 **Q42 확정 → 착수 가능**, 발송 경로 DB 쓰기 측정 동반 · 1a-2 는 **#1 + Q16·Q17·Q18** · 1a-1·1a-3·1a-4 는 Q10 외 다수):

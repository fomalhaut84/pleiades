...
**1a-0 은 착수 가능하다 — Q42 확정(#29):** 검증은 로컬 `repos/*` + **서버 병행 인스턴스(읽기 전용 DB 롤, 별도 포트·pm2, 봇 미기동, cron off)**. 착수 시 1a 발송 경로의 DB 쓰기 지점을 먼저 잰다(003 §10-1). 서버 작업은 별도 승인. **1a-1 부터는 추가로 하네스 통합(#1)이 선결**이고 #1 은 방향만 확정됐다 — 1a-1 이 이미 두 저장소 코드를 흡수한다.

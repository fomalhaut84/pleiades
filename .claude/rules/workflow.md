...
> 봇 장애로 **안 올 수 있다.** 그 경우 9-4 의 종료 조건 *"봇 P0/P1 = 0"* 은 **판정 불능**이지 통과가 아니다. 대체 경로:
>
...
>
> **PR body 표기(9-6)는 고정 문구다:** `봇: 미실행 (<사유>, YYYY-MM-DD)` — 사유는 `쿼터 소진` · `머지까지 미게시` · `봇 장애` 중 하나.
> `봇 P0/P1 = 0/0` 을 **적지 않는다** — 그 값은 없다. **소급 리뷰는 하지 않는다**(머지된 PR 에 `@codex review` 는 무의미) —
> 기록이 대체하고, 그 코드는 다음 PR 의 봇 리뷰 범위에서 다시 보인다. 되돌리기: 문구 (즉시).

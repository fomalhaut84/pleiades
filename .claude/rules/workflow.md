...
#    봇이 안 오면 머지 보류 — 9-3 "봇이 돌지 않을 때" 표의 릴리즈 행 (#14)
...
> **봇이 돌지 않을 때 (#14 · 2026-09-08).** 봇은 쿼터 소진(2026-09-04 PR #13) · 미게시(2026-09-08 PR #33 — 머지까지 리뷰 0건) ·
> 봇 장애로 **안 올 수 있다.** 그 경우 9-4 의 종료 조건 *"봇 P0/P1 = 0"* 은 **판정 불능**이지 통과가 아니다.
>
> **"불가" 판정은 머지 전에 관측 가능해야 한다 (PR #35 Codex P1).** *"머지까지 미게시"* 는 머지한 뒤에야 참이 되므로
> 9-6 body 를 머지 전에 확정할 수 없다. 컷오프: **PR 오픈(또는 마지막 `@codex review`) 후 30분 안에 리뷰·👍 가 없으면
> `@codex review` 를 1회 요청하고, 그 뒤 30분에도 없으면 "미게시" 로 판정**한다 — 두 시각을 PR body 에 적는다.
> 쿼터 소진·봇 장애는 봇의 오류 응답이 곧 판정이다(그 시각). 대체 경로:
>
> | 변경 성격 (9-0) | 봇 불가 시 완료 판정 |
> |---|---|
> | **self-review 경로** (문서·하네스·config 소규모) | self-review + 8절 검증으로 완료. **봇을 기다리지 않는다** |
> | **에이전트 필수 경로** (패키지 코드·대상 저장소·공개 인터페이스·규모·보안) | **9-1 사전 에이전트 리뷰 결과(critical/major = 0)가 완료 판정**이다. 봇 없이 사전 리뷰도 생략한 PR 은 머지하지 않는다 |
> | **서비스 핫픽스** (긴급 수정 절 4번) | 위 필수 경로와 같다 + **사용자가 "봇 없이 머지" 를 명시 승인**한다. 실서비스로 직행하므로 묵시 승인은 없다 |
> | **릴리즈 PR** (`dev` → `main` · 릴리즈 전략 절) | **대체 없음 — 봇 회복까지 머지 보류**(#14 선택지 ②). 릴리즈는 실서비스로 직행하고 핫픽스와 달리 긴급성이 없다. 보류가 실서비스 장애를 연장하는 경우에만 핫픽스 행(사전 에이전트 리뷰 + **사용자 명시 승인**)으로 내려간다 (PR #35 Codex P1) |
>
> **PR body 표기(9-6)는 고정 문구다:** `봇: 미실행 (<사유>, YYYY-MM-DD HH:MM)` — 사유는 `쿼터 소진` · `미게시 (오픈 HH:MM · 재요청 HH:MM)` · `봇 장애` 중 하나.
> `봇 P0/P1 = 0/0` 을 **적지 않는다** — 그 값은 없다. **그 PR 에서는 9-1 사전 에이전트 리뷰가 최종 리뷰다** (PR #35 Codex P2 —
> 머지된 코드는 다음 PR 의 diff 에 없으므로 *"다음 PR 봇 범위에서 다시 보인다"* 는 성립하지 않는다). 머지된 PR 에 `@codex review` 는
> 무의미하므로 소급 리뷰도 없다. 필수 경로 코드가 봇 없이 머지되면 **후속 이슈에 그 파일을 적고**, 그 파일을 건드리는 다음 PR 의
> 9-1 focus 에 넣는다 — 그것이 유일한 재검토 경로다. 되돌리기: 문구 (즉시).
...
봇이 돌지 않았으면 (9-3 봇 불가 표의 경로여야 한다 · PR #35 Codex P2 ×2):
- 에이전트 필수 경로: `✅ 코드 리뷰 통과 (사전 critical/major: 0건 · 봇: 미실행 (미게시 (오픈 HH:MM · 재요청 HH:MM), YYYY-MM-DD HH:MM) · 검증 재통과)`
- self-review 경로: `✅ 코드 리뷰 통과 (self-review · 봇: 미실행 (<사유>, YYYY-MM-DD HH:MM) · 검증 통과)` — **사전 건수를 적지 않는다**(하지 않은 리뷰다)
사유 문구와 시각은 9-3 의 고정 형식 그대로 옮긴다 — 컷오프를 지켰다는 증거가 그 시각이다.
...
- **최종 상태** — **0 을 요구하는 것은 `critical`/`major` 와 봇 `P0`/`P1` 뿐이다.**
  ...
  봇이 돌지 않았으면 `봇 P0/P1` 자리에 **9-3 고정 형식 그대로** `봇: 미실행 (<사유>, YYYY-MM-DD HH:MM)` 을 쓴다 — 미게시면 사유 안의 오픈·재요청 시각까지 (9-3 봇 불가 절 · #14 · PR #35 Codex P2). `0/0` 으로 적지 않는다. self-review 경로면 `사전 critical/major` 항목 자체를 `self-review` 로 대체한다
...
4. **봇 리뷰가 여기서 돈다** (9-3 은 PR 오픈 후에만 실행된다). 봇 **P0·P1** → **반드시 수정**
   — **봇이 안 오면** 9-3 봇 불가 표의 핫픽스 행: 사전 리뷰 결과가 판정이고 **사용자가 "봇 없이 머지" 를 명시 승인**해야 한다 (#14)

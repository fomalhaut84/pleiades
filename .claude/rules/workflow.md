...
**`grep` 에는 반드시 binary-safe 옵션** — 도구마다 이름이 다르다:
  ...
  **`--binary-files=text` 는 절반만 막는다** — 이 환경의 `grep` 은 **ugrep 래퍼로 `.gitignore` 를 따른다**
  (005 §4-11 · 2026-09-07 실측). ignored 디렉터리(fit `.claude/` 등)는 통째로 0건이 된다. ignored 경로를
  포함하는 측정은 **`--no-ignore-files` · `/usr/bin/grep` · 경로 직접 지정** 중 하나를 반드시 쓴다.
  `git grep` 은 인덱스만 보므로 ignored 경로에는 원리상 쓸 수 없다
- **열거와 합계가 함께 있으면 합계를 신뢰하지 말고 열거를 센다** (005 R1). 요약 숫자에는 열거의 출처
  (파일 · 절 · 행 범위)를 붙인다. **대장(숫자 표)에 없는 숫자를 본문에 새로 만들지 않는다.** 같은 숫자를
  문서 안에서 두 번 적지 않는다 — #11 유형(합계≠열거)이 005 초안에서 3라운드 연속 6건 나왔고, 대장 형식을
  도입한 2회차부터 0건이었다
...
- 모든 옵션·단계에 **되돌리기 등급 + 되돌리는 행위 + 소요**를 병기한다. 예외 없다
  - **실작업 시간 기록이 없는 단계는 소요를 등급으로 갈음하고 그 사실을 적는다** (005 R2). 이 저장소에는
    시간 기록이 없다 — 추정 시간을 만들어 적지 않는다
- 등급: **즉시** / **중간** / **높음** / **편도**

...
봇이 돌지 않았으면 `✅ 코드 리뷰 통과 (사전 critical/major: 0건 · 봇: 미실행 (<사유>, 날짜) · 검증 재통과)` — 9-3 봇 불가 표의 경로여야 한다.
...
- **최종 상태** — **0 을 요구하는 것은 `critical`/`major` 와 봇 `P0`/`P1` 뿐이다.**
  ...
  봇이 돌지 않았으면 `봇 P0/P1` 자리에 **`봇: 미실행 (<사유>, YYYY-MM-DD)`** 를 쓴다 (9-3 봇 불가 절 · #14). `0/0` 으로 적지 않는다

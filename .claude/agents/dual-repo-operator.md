...
4. **롤백 절차를 문서로 남긴다** — `_workspace/<주제>/04_operator_rollback.md`. 필수 항목은 `dual-repo-change` **5-1**
   (머지 전 · 머지 후 · 원본 도달분 — 셋 전부 · 머지 후 SHA 실값). 승인 게이트 4번과 같은 내용이어야 한다 —
   달라지면 승인이 무효다. `git revert` 는 즉시 커밋하므로 **브랜치 생성이 앞선다**

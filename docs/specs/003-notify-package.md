...
| | 질문 | 무엇을 가르는가 | 우선 |
|---|---|---|---|
| **Q10** | myFitness 아웃바운드 **절단 → 분할**을, 그리고 **plain 폴백 정본**을 1a 에서 받나 | 받으면 지금 버려지던 4096 초과분이 살아나 **사용자에게 메시지 수가 늘어난다**(관측 가능한 동작 변경). 안 받아도 **1a 가 무동작-변경이 되지는 않는다** — fin `rsu.ts:184` 가 raw `sendMessage` 라
파사드 흡수만으로 재시도 ×4 + plain 폴백이 **무조건 새로 붙고**(발견 20 정정), `lastError` sanitize 도
Q19 에 따라 붙는다. 안 받으면 **fit 쪽 손실만 지속**된다. plain 폴백(`telegram.ts:69` vs `markdown.ts:96-103`)도 **어느 쪽을 골라도 한쪽 저장소의 출력이 바뀌는** 같은 성격의 변경이라 함께 답한다 | **높음 — 1a 착수 전** |
...
되돌리기가 **즉시**이므로, **위 7개**(Q10 · Q16 · Q17 · Q18 · Q19 · Q25 · Q26) 중 하나도 답을 받지 못한 상태에서도 착수 가능하다.
...
| **Q10 에 답이 나올 때** | **fit 쪽** 조건부 항목이 붙는지가 확정된다. **1a 전체가 "동작 변경 0" 이 되는 것은 아니다** — fin `rsu.ts:184` 의 재시도·폴백 신설은 Q10 과 무관하게 붙고, `lastError` sanitize 는 Q19 에 달렸다 (PR #9 Codex 리뷰 P2) |
...
> **1a-4 착수 전에 남은 정적 측정은 0건**이고, 남은 것은 **결정 7개**다(§10-1: Q10 · Q16 · Q17 · Q18 · Q19 · Q25 · Q26).

...
>
> **코어 정규식 확정 (PR #39 Codex P2 반영 · 2026-09-09).** 위 단서만으로는 코어가 어느 수량자를 구현할지가 비어 있어 *"동작 변경 0"* 이 판정 불능이었다.
> **코어는 fin 의 `/<[^>]+>/g` 를 구현한다.** 근거: 폴백 경로에 **도달하는 모듈이 fin 11 · fit 0**(C-4)이므로 fin 정규식을 고르면
> fin 11 모듈은 **무조건 변화 0** 이고, 변화 후보는 fit 의 `sendToAll` 4 호출에서 **리터럴 `<>` 가 입력에 실제로 나타날 때**(미측정)로 한정된다 —
> fit 정규식을 고르면 같은 조건이 fin 11 모듈에 걸린다. 즉 **관측 변경의 잠재 범위를 최소화하는 쪽**이다.
> 이 확정은 오케스트레이터 판단이며 사용자 확정 Q10-P ①(태그-only)의 하위 구현 세부다 — 뒤집으려면 코어 1줄(되돌리기 **즉시**).
...
> **정정 (2026-09-10 · 이슈 #47 · 1a-1 착수) — 정본 시그니처가 참조만 하고 정의하지 않은 타입을 1a-1 이 채웠다.**
>
> 이 절 맨 위 코드 블록은 `Content` 를 **정의하지 않은 채** 포트·파사드 양쪽에서 참조했다. 1a-1 구현 계획
> (`_workspace/1a-1/01_plan_1a1.md` §0 S-1·S-2·S-3 · §2 · §8 D-1~D-4 · 2회차 · 착수 직전 재감사
> `_workspace/1a-1/03_auditor_1a1plan.md` · **사용자 승인 2026-09-10**)이 그 빈칸과 딸린 넷을 채웠다.
> **정본은 "이 절 맨 위 코드 블록 + 이 정정" 이 함께다** — 위 블록은 다시 쓰지 않는다.
>
> **① `Content` — 정의가 0건이었다** (계획 S-1 · 감사 §4 · 결정 **D-1**).
> `docs/` 전체에 `interface Content|type Content` 가 **0건**이고, 003 의 `Content` 는 **8곳 전부 참조**다
> (`:112` `:656` `:661` `:798` `:803` `:834` `:870` `:898` — 행 번호는 이 정정 블록 삽입 **이후** 기준). 1a-1 이 정의한다:
>
> ```ts
...
> ```
>
> `components` 는 §5-1 의 *"`rsu.ts` 키보드 → 포트에 `components` 옵션(텔레그램 구현만)"* 이 요구한 자리다.
> **`format` 은 코어가 만든다** — 파싱 실패 폴백이 같은 본문을 `'plain'` 으로 재전송한다. 근거는 §4-1(b)
> *"수신자 목록 해석 · N건 루프 · 결과 집계 · 재시도 백오프 · 파싱 실패 폴백은 전부 **채널 무관 로직**"* 이다.
> 즉 `format` 은 §4-3 제약 3 이 `Transport` 직전으로 못박은 **변환**이 아니라 **폴백 상태**이므로 제약 3 을 어기지 않는다.
> **되돌리기: 즉시** (1a-3 착수 전까지 · 이후 **중간** — §8-1) — 타입 재정의.
>
> **② `Transport.maxLength: number` 를 포트에 추가** (계획 S-3 · 결정 **D-2**).
> §4-3 제약 2 가 허용한 **둘째 형태**(*"포트가 '변환 후 길이 예산'을 코어에 알려주는 형태"*)를 택했다.
> 근거(S-3): fin `telegram.ts:61-73` 은 **청크 단위로** 재시도·폴백한다. 분할을 `Transport.send` 안에 두고
> 재시도를 코어에 두면 `send` 가 *부분 성공 후 throw* 하는 **비멱등** 연산이 되어 **이미 나간 청크가 중복 전송**된다.
> 그래서 **소유권 규칙을 포트 계약에 명문화한다 — 섞을 수 없다**(감사 §3(다)):
>
> | `maxLength` | 분할 | 재시도·폴백 | `send` 의 단위 |
> |---|---|---|---|
> | **유한** (`TelegramTransport` = 4096) | **코어** | **코어** | **청크 1건 = 멱등 단위** |
> | **`Infinity`** | 어댑터 | 어댑터 | 호출 1건. **코어는 재시도하지 않는다** |
>
> `Infinity` + 코어 재시도의 조합은 제약 2 의 **첫째 형태**이고 그것이 위에서 반증된 조합이다 —
> 그 탈출구를 열어두면 S-3 의 결함이 **1b(Discord) 로 그대로 옮겨간다**(감사 §3(다)).
> **되돌리기: 즉시** (1a-3 전 · 이후 **중간** — §8-1) — 포트 필드 1개 삭제 + 분할 위치 이동.
>
> **③ `NotifierConfig.transport: TransportRef | Partial<Record<Route, TransportRef>>`** (결정 **D-4**).
> §1-4 **요구사항 1**(*"`Route → Transport` 매핑을 설정으로 뺀다"*)의 이행이다 — **이미 확정된 정본 요구사항이라
> 새 결정이 아니다.** 안 넣으면 §1-4 가 예고한 대로 **1b 되돌리기가 "env 플립" → "코드 revert + 재배포"로
> 한 등급 오른다**(그때는 소비자 둘 다 config 를 박은 뒤다). `TransportRef = Transport | (() => Transport)` 가
> §4-3 **제약 1**(지연 생성)을 만족한다. 대상은 있는데 그 Route 의 transport 가 없으면 `notify` 가 **throw** 한다
> (조용한 무전송보다 낫다). **되돌리기: 즉시** (config 한 칸 · **포트 무변경** · 1a-3 전 · 이후 중간 — §8-1).
>
> **④ `TelegramApi` — grammy 를 참조하지 않는 구조적 타입** (계획 S-2 · 결정 **D-3**).
>
> ```ts
> export interface TelegramApi {                    // 메서드 단축 문법 필수
>   sendMessage(chatId: string | number, text: string,
>               other?: { parse_mode?: string; reply_markup?: unknown }): Promise<{ message_id: number }>;
> }
> ```
>
> 근거: grammy 를 **어디에도 선언하지 않고** `import type` 만 쓰면 소비자의 임시 클론 `prepare`(`tsc -p packages/notify`)가
> **TS2307 → `npm install` 실패**한다(감사 케이스 B 재현 — 1a-0 `exclude` 누락과 같은 경로, 두 실서비스 `npm ci` 동시 파손).
> 루트 devDep·peerDep 도 `prepare` 를 통과하지만(준비 명령에 **`--include=peer`** 가 있다 · 케이스 C·D),
> **구조적 타입이 가장 싸다** — 소비자 `npm ci` warm **1.56 s** · cold **2.04 s / 캐시 19 M** · 소비자 트리 **20 K**
> (devDep 1.88 s / 2.92 s / 22 M · peerDep 는 범위가 소비자 설치본보다 좁으면 `npm ci` 가 **ERESOLVE** 로 죽는다 · 케이스 F).
> **형태에 두 제약이 있다**(감사 V1·V8 · fin grammy **1.44.0** · fit **1.42.0** 양쪽 프로브):
> `parse_mode` 는 **`string`** 이어야 한다 — `'HTML'` 리터럴은 grammy `ParseMode` 보다 **좁아** `TS2322` 이고,
> **메서드 단축 문법**이어야 한다 — 프로퍼티(화살표) 문법은 파라미터 **양변성이 없어** 역시 `TS2322` 다.
> **되돌리기: 즉시** (1a-3 전 · 이후 **중간** — §8-1) — 인터페이스 1개 교체 또는 루트 선언 추가.
>
> **⑤ `csvEnv(name, { numericOnly? })`** (감사 §5d).
> 수신자 파싱은 fit 규칙(`split·trim·filter` · 문자열 유지)을 기본으로 하되, fin **5지점**의
> `.map(Number).filter(!isNaN)` **탈락 규칙을 옵션으로 보존**한다. 비숫자 토큰의 탈락 여부가 `total` 의 값을 바꾸고,
> 그 값은 **DB 컬럼 `AlertHistory.recipientCount` 3곳 + API 응답 필드 `totalChats` 1곳**이다(발견 19 —
> *"값의 의미가 바뀌면 안 된다"*). **env 실값에 비숫자 토큰이 있는지는 미확인**(`.env` 실값 열람은 측정 규율상 금지).
> 옵션을 안 두면 1a-4 에서 **DB 값의 의미 변경**으로 되돌아온다. **되돌리기: 즉시** (코어 옵션 1개 · 1a-3 전 · 이후 중간).
...
> **정정 (2026-09-10 · 이슈 #47 · 1a-1) — fin 측 관측 변경 후보가 하나 더 있다: 폴백 트리거 축소.**
> 바로 위 정정 ①은 fin 측 관측 가능한 동작 변경을 **2건**(`rsu.ts` 전송 신뢰성 · `lastError` sanitize)으로 열거했다.
> **3번째 후보가 있다** — §3-1 정본이 파싱 실패 **폴백 판정자**를 fit `isHtmlParseError`(공유 정규식)로 골랐기 때문이다.
>
> | | 판정자 | 폴백이 도는 범위 |
> |---|---|---|
> | fin 현행 | 로컬 `isParseError` (`telegram.ts:79-88`) | **`error_code === 400` 전체** + `"can't parse"` 문자열 |
> | 1a-1 코어 | fit `isHtmlParseError` (`error.ts:82-85`) | `can't parse entities` · `Bad Request:.*entit` 만 |
>
> 즉 fin 은 지금 **비-파싱 400**(예: *message is too long*)에서도 태그를 벗겨 plain 으로 재전송하므로
> **그 재전송이 성공할 수 있는 경로가 현존한다.** 1a-1 코어는 그 경로를 없앤다 — 에러가 그대로 올라간다.
>
> **빈도는 미측정이다.** 절단·분할·폴백은 로그를 남기지 않아 양쪽 **폴백 발생 로그 0건**이고(measured-facts C-1),
> 계측 코드 없이는 잴 수 없다(그러려면 두 실서비스에 쓰기·배포가 필요하다 — 1a-1 범위 밖).
> 따라서 정정 ①의 열거는 **2건 → 3건 후보**로 읽고, **"관측 변경 0" 을 주장하지 않는다.**
> 되돌리기: **즉시** — 코어 판정자 1함수를 fin 형태로 되돌린다 (1a-3 착수 전까지 · 이후 **중간** — §8-1).
...
> **집행 (2026-09-11 · #58 · 계획 `_workspace/1a-2/01_plan_1a2.md` 2회차 · 재감사 정정 10 · 블로커 3).** 사용자 확정 **Q16 승인 · Q17 `src/**/__tests__/` · Q18 devDeps**(전부 권고안) + **G-1**(1a-3 교체 대상 `send.ts` 직접 baseline 포함) + **G-2**(머지 후 원본 하네스 동기화 — fit `git archive` · fin #51 은 모드 S 미러 PR).
...
> **이행 완료 (2026-09-11 · #58 · 1a-2 재감사).** 아래 U3 은 **해소됐다** — 스크래치 전체 설치(682 packages)에서 vite 6.4.3 이 top-level esbuild 0.28.1 을 해석하고
> `npx vitest run` 이 돈다(measured-facts "1a-2 집행 실측" §3). 대안(`$esbuild` 리터럴화 · vitest 하향)은 쓰지 않았다. U2 는 1a-2 E5(실트리 `npm run lint`)에서 확정 — 결과는 같은 절 §8.
...
>
> **가드 범위 정정 (PR #39 Codex P2 반영 · 2026-09-09) — B′ 단독은 `sensitiveLogs` 의 안전한 대안이 아니다.**
> B′ 는 `lastError` **4지점**만 마스킹한다. 그런데 이 문서는 발견 20 의 **A 유형 `console.error(raw error)` 12건**(위 §3 표 · `:454-458`)을
> 파사드가 흡수하고 `label` 컨텍스트(Q26 ①)로 **그 로깅을 보존**한다고 정했다. 따라서 `sensitiveLogs: true` 에서 `HttpError` 가 봇 토큰 URL 을 담으면
> 영속·UI 5 표면을 막아도 **파사드의 에러 로그 경로로 프로세스 로그에 토큰이 남는다.**
> 별건 가드의 형태는 둘 중 하나여야 한다: **(가) `sensitiveLogs` 를 금지하는 룰**(코드 0 · 되돌리기 **즉시**), 또는
> **(나) 파사드의 에러 로깅 1곳에 `sanitizeMessage` 를 적용 + B′ 4지점** — (나)는 파사드 코드 1곳이라 **1a-1 코어 범위**이며 되돌리기 **즉시 (1a-3 전까지 · 이후 중간 — §8-1)**.
> **(나)를 고르면 Q19 C 의 "포트 raw" 는 유지된다** — 마스킹은 `deliveries[].error` 가 아니라 **파사드가 찍는 로그 문자열**에만 걸리기 때문이다.
>

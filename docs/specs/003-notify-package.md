...
| `scheduler.ts:21` | 가드 1 + **14 cron 콜백 전달** + `:108` 목록 필요 | **✕** (Q25 에 종속) |
...
| **Q25·Q26 에 답이 나올 때 (2026-09-04 신설)** | **§4-2 시그니처가 확정된다.** 1a-1 은 이 답 없이 시작할 수 없다 — 나중에 바꾸면 1a-4 호출부 20곳이 재편집 대상이 된다 |
